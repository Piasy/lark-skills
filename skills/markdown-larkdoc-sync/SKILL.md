...
- `bin/resolve_doc_key.py`、`bin/find_last_sync_commit.py`、`bin/extract_markdown_body.py`、`bin/fetch_open_comments.py`、`bin/resolve_all_comments.py`、`bin/create_sync_commit.py` 都应被视为工作流标准入口。
...
3. 调用 `bin/find_last_sync_commit.py`。
4. 读取远端正文，并调用 `bin/fetch_open_comments.py` 获取当前全部未解决评论。
5. 做正文三方合并。
